...
use App\Models\LogNowpayment;
class Nowpayments {
	private $session, $token, $url;

	//$this->url = 'https://api-sandbox.nowpayments.io/v1/';
	...
	function __construct($token=null, $url=null) {
		...
		if(!$token){
			$this->token=env('NOWPAYMENTS_TOKEN');
		}else{
			$this->token=$token;
		}
	...
	private function Call($method, $endpoint, $data = []) {
			...
			case 'GET':
				...
				if(!empty($data)) {
					if(is_array($data)) {
						...
						curl_setopt($ch, CURLOPT_URL, $this->url.$endpoint.'?'.$parameters);
					} else {
						if($endpoint == 'payment') curl_setopt($ch, CURLOPT_URL, $this->url.$endpoint.'/'.$data);
					}
				} else {
		...
		if($method=='POST' && env('NOWPAYMENTS_LOGS')){
			LogNowpayment::create([
				'post'=>$data,
				'json'=>$response,
				'mark'=>1
			])->save();
	...
	*/
	...
	public function fullcurrencies(){
		return $this->Call('GET', 'full-currencies', null);
	}
